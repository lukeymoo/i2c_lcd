...
void twi_init(void)
{
	...
	TWCR = (1 << TWEA) | (1 << TWINT) | (1 << TWEN) | (1 << TWIE);  // enable i2c
	return;
}
...
{
	bus.retval = 0;
	TWDR = (bus.r_addr << 1) | 0x01;
	TWCR = (1 << TWEN) | (1 << TWINT); // hardware operation
	while(!(TWCR & (1 << TWINT))); // wait
	// Hold status register in bus struct
	bus.retval = TWSR;
...
/**
  ...
  */
uint8_t _send_byte(uint8_t byte)
{
